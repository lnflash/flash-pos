...
7. **Nutshell checkstate takes `Ys`** — the **full compressed-point hex** (02/03 prefix + 32-byte X, exactly what cashu-ts derives internally). A stripped X-only hex reads as an unknown Y and answers UNSPENT for proofs that are genuinely spent — this bit the milestone session's own manual probes (a `.slice(2)` on the hex) and produced an hour of wrong conclusions; the adapter's `checkState` is correct because cashu-ts derives the Y internally. Unknown Ys also read UNSPENT — checkstate cannot prove a secret is *known*, only whether a Y is spent.
...
**Mint ground truth**: POST `https://forge.flashapp.me/v1/checkstate` with `{"Ys": [hashToCurve(secret).toString('hex')]}` — the **full compressed-point hex** (33 bytes; do NOT strip the 02/03 prefix — X-only hex reads as unknown Y and always answers UNSPENT). → `{states: [{state: 'SPENT'|'UNSPENT'}]}`. Space calls ≥60 s under throttle; `{"detail":"Rate limit exceeded."}` means back off, don't retry.
...
1. **Pixel's pending 8-sat entry** (resolved at the mint, resolving in the app): the entry's proof (and the 16-sat sibling) **is SPENT at the mint** — verified with correctly-encoded checkstate (`f0e211a`+ correction below) after the hand settlement. The drain's pending-checkState pass (`cfff018`) settles it as money-received on the next run; a "Settle now" press that returned "nothing settled to sweep" had raced an in-flight backoff run or hit the throttle before reaching the entry. If the banner somehow still holds it: read the entry's `status`/`lastError` from the Pixel's store (`@cashu_settlement_queue` via AsyncStorage) — a `failed` state here is retired with the banner's "Settled elsewhere". **The deeper fix** (next milestone): a legacy-variant shim — on swap rejection, retry once with the tagless secret variant (the app holds the nonce; only the tags differ) — this would let the *app* recover old-path proofs instead of an operator. Note the drain cannot do this shim (the stored witness signs the canonical variant's hash); it is an `executeCharge`-only recovery since the card must be in the field to re-sign.
